          ...
          <div class="card card-">
              ...
              if (isset($_POST['submit'])) {
               ...
               if ($check ==1) {
                ...
                echo '<script> window.location="add_purchase?purchaseID='.$purchase_ID.'&invoice_no='.$invoice_no.'&date='.$date.'&supplier='.$supplier_ID.'&det='.$purchase_details.'" </script>';;
                 ...
                 $create= createTempPurchaseHistory($purchase_ID,$product_ID,$product_name,$supplier_ID,$quantity,$supplier_price,$discount,$total_price,$batch_Id);
                if ($create =='success') {
                  //echo '<script> window.location="add_purchase?purchaseID='.$purchase_ID.'&invoice_no='.$invoice_no.'&date='.$date.'&supplier='.$supplier_ID.'&det='.$purchase_details.'" </script>';;
                }
                  ...
                  if ($create =='success') {
                    ...
                    if ($getdatas !=0) {
                       ...
                       foreach ($getdatas as $getdata) {
                          ...
                          $currentStockIN=getStockIN($product_ID);
                          $new_stockin= $currentStockIN + $quantity;
                          updateStockIN($product_ID,$new_stockin);
                       ...
                       $updt=updateQuantityAfterPurchase($product_ID,$new_quantity);
                         ...
                         $activity='Made a purchase with an invoice no '.$invoice_no;
              ...
              if (isset($_GET['med']) && !empty($_GET['med'])) {
               ...
               if (isset($_POST['cancelPurchase'])) {
                 ...
                 if ($dels =='success') {
                    echo '<script> window.location="add_purchase" </script>';
                 }
              }
              ?>
              <form method="POST" action="add_purchase?purchaseID=<?php echo $purchase_ID ?>&invoice_no=<?php echo $invoice_no?>&date=<?php echo $date ?>&supplier=<?php echo $supplier_ID ?>&det=<?php echo $purchase_details ?>" class="form form-responsive" enctype="multipart/form-data">
                <div class="card-body">
               ...
               <?php }
                ...
                <table class="table" >
                  ...
                  <tr>
                    ...
                    <td><a href="delete_temp_purchase_table?id=<?php echo $getdata->id ?>&purchaseID=<?php echo $purchase_ID ?>&invoice_no=<?php echo $invoice_no?>&date=<?php echo $date ?>&supplier=<?php echo $supplier_ID ?>&det=<?php echo $purchase_details ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete?')"><i class="nav-icon fas fa-trash"></i></a></td>
                  </tr>
                      <?php }
                   }
                  ?>
                  ...
                  <tr>
                    ...
                    <td><input type="number" class="form-control" id="amount_paid" min="0" name="amount_paid"   placeholder="amount Paid" onchange="calBalanceAndDue(this.value)" onkeyup="calBalanceAndDue(this.value)" value="0"  width="20px" required></td>
                  </tr>
                  <tr>
                    <td colspan="4"></td>
                    <td><b id="showBalance">Balance</b></td>
                    <td><input type="number" class="form-control" id="bal" min="0" name="bal"  disabled>
...
<script type="text/javascript">
 function getproductDetails(med){
  var sale_date=document.getElementById('purchase_date').value;
    ...
    window.location="add_purchase?med="+med+"&supplier="+supplier_ID +"&purchaseID=<?php echo $purchase_ID ?>&invoice_no="+invoice +"&date="+sale_date+"&det="+details;
 }
</script>
<script type="text/javascript">
  function getproductbySupplier(supplier){
    ...
    var company=document.getElementById('supplier_ID').value;
     var invoice=document.getElementById('invoice_no').value;
    window.location="add_purchase?supplier="+supplier +"&purchaseID=<?php echo $purchase_ID ?>&invoice_no="+invoice +"&date="+sale_date+"&det="+details;
