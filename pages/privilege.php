        ...
        <div class="row">
          ...
          <div class="card card-info">
              ...
              if (isset($_POST['submit'])) {
                ...
                $role_ID=$_GET['role'];
               $del=deletePreviousPrivilegis($role_ID);
               if (isset($_POST['privilege']) && !empty($_POST['privilege'])) {
               $privilege_IDs=$_POST['privilege'];
                foreach ($privilege_IDs as $privilege_ID){ 
                   
             $save= createRole_Privileges($role_ID,$privilege_ID);
              
                 if ($save='success') {
                  $role_name=getRolesByID($role_ID);
                   
                   $activity='Updated Privileges for '. $role_name. 'Role';
                       createActivity_logs($activity);
                         echo '<script> alert("updated Successfully") </script>';
                 
                     echo '<script> window.location="privilege?role='.$role_ID.'" </script>';
              ...
              <form method="POST" action="privilege">
                <div class="card-body">
                  ...
                  <select class="form-control select2" style="width: 100%;" onchange="getPrivilegesFunc(this.value)">
                     <?php
                  if (isset($_GET['role'])) {
                    $role_ID= $_GET['role'];
                  $selectedRoles=getRolesByID($role_ID);
                  if ($selectedRoles !=0) {
                    foreach ($selectedRoles as $selectedRole ) { ?>
                     <option value="<?php echo $selectedRole->role_ID ?>"><?php echo $selectedRole->role ?></option>
                    <?php }
                  }
              ...
              <script type="text/javascript">
                function getPrivilegesFunc(role){

                  window.location="privilege?role="+role 
                }
              </script>
          ...
          <div class="col-md-12">
              <div class="card">
              <div class="card-header">
                <h3 class="card-title"><b>ROLE</b> :
                  <b style="color:#17A589">
                    <?php
                  if (isset($_GET['role'])) {
                      ...
                      $role_ID= $_GET['role'];
                 echo $selectedRoles=getRolesByID($role_ID);
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <form method="POST" action="privilege?role=<?php echo $_GET['role']?>">
                <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                      $getPrivileges=getPrivilegesAll();
                      $role_ID='';
                      if (isset($_GET['role'])) {
                        $role_ID=$_GET['role'];
                      ...
                      if ($getPrivileges !=0) {
                        ...
                        $x++;
                       $rolePrivileges= getRolePrivilegesByRoleAndPrivilegesID($role_ID,$getPrivilege->id);
                        if ($rolePrivileges !=0) { 
                          
                          foreach ($rolePrivileges as $rolePrivilege ) {
                            if ($rolePrivilege->privilege_ID ==$getPrivilege->id && $rolePrivilege->role_ID==$role_ID ) { ?>
                              <tr>
                            <td><div class="icheck-primary d-inline">
                        <input type="checkbox" id="<?php echo $getPrivilege->id ?>" name="privilege[]" value="<?php echo $getPrivilege->id ?>" checked>
                        <label for="<?php echo $getPrivilege->id ?>">
                          <?php echo $getPrivilege->privilege ?>
                        </label>
                      </div></td>
                       ...
                       <?php  }else{
                         ?>
                          <tr>
                            <td><div class="icheck-primary d-inline">
                        <input type="checkbox" id="<?php echo $getPrivilege->id ?>" name="privilege[]" value="<?php echo $getPrivilege->id ?>">
                        <label for="<?php echo $getPrivilege->id ?>">
                          <?php echo $getPrivilege->privilege ?>
                        </label>
                      </div></td>
