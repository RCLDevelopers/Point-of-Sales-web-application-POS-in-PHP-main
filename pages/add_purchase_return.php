...
<?php
$role_ID=$user_role;
$privilege_ID='Add Purchase Return';
        ...
        <div class="row mb-2">
          ...
          <div class="card card-default">
               <div class="card-header" style="color: #17A589;font-size:">
                ...
                <div class="row">
                  <div class="col-md-4" >
                      <h3 class="card-title" style="color: #17A589;font-size:"><i class="nav-icon fas fa-"></i> PURCHASE RETURN FORM</h3>
                  </div>
                  <div class="col-md-8">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          ...
          <div class="card card-info">
              ...
              $purchase_ID='';
               if (isset($_GET['purchaseID'])) {
                ...
                foreach ($getPurchases as $getPurchase) {
                 $date=$getPurchase->purchase_date;
                 $invoice_no=$getPurchase->invoice_no;
                 $purchase_details=$getPurchase->purchase_details;
             ...
             $create=createPurchaseReturn($purchaseReturnID,$purchase_ID,$supplier_ID,$invoice_no,$total_deduction,$return_total);
            $createDets='';
                if ($create =='success') {
                   $getdatas=getTempPurchaseReturnDetailByReturnID($purchaseReturnID);
                  if ($getdatas !=0) {
                    foreach ($getdatas as $getdata) {
                     $createDets=createPurchaseReturnDetails($purchase_ID,$purchaseReturnID,$getdata->product_ID,$getdata->supplier_price,$getdata->return_quantity,$getdata->deduction_percentage,$getdata->total_deduction,$getdata->total_return);
                     updatePurchaseAfterPurchaseReturnByBatch($getdata->product_ID,$getdata->batch_id,$getdata->return_quantity);
                    $activity='Made a Purchase return with an invoice no '.$invoice_no;
                          createActivity_logs($activity);
                      
                    }
                    if ($createDets =='success') {
                      $del=deleteTempPurchaseReturnTableByID($purchaseReturnID);
                      if ($del =='success') {
              ...
              <form method="POST" action="privilege">
                <div class="card-body">
                  ...
                  if (isset($_GET['purchaseID'])) {
                    $purchase_ID= $_GET['purchaseID'];
                  $selectedinvoices=getPurchaseByID($purchase_ID);
                  if ($selectedinvoices !=0) {
                    foreach ($selectedinvoices as $selectedinvoice ) { ?>
                     <option value="<?php echo $selectedinvoice->purchase_ID ?>"><?php echo $selectedinvoice->invoice_no ?></option>
                    <?php }
                  }
                ...
                ?>
                    <?php
                     $getPurchases=getPurchaseALL();
                     if ($getPurchases !=0) {
                        foreach ($getPurchases as $getPurchase ) {?>
                          <option value="<?php echo $getPurchase->purchase_ID ?>"><?php echo $getPurchase->invoice_no ?></option>
                        <?php }
                     }
                    ?>
                  </select>
              ...
              <script type="text/javascript">
                function getInvoiceFunc(purchaseID){

                  window.location="add_purchase_return?purchaseID="+purchaseID 
                }
              </script>
          ...
          <div class="col-md-12">
              ...
              <div class="card-header">
                    ...
                    <?php
                    $purchaseReturnID='';
                    if (isset($_GET['purchaseReturnID']) && !empty($_GET['purchaseReturnID'])) {
                      $purchaseReturnID=$_GET['purchaseReturnID'];
                    }else{
                      $purchaseReturnID=generatePurchaseReturnID();
                    }
                  if (isset($_GET['purchaseID'])) {
              ...
              <!-- /.card-header -->
              <div class="card-body">
                 <form method="POST" action="add_purchase_return?purchaseID=<?php echo $purchase_ID ?>&invoice_no=<?php echo $invoice_no?>&date=<?php echo $date ?>&supplier=<?php echo $supplier_ID ?>&det=<?php echo $purchase_details ?>&purchaseReturnID=<?php echo $purchaseReturnID ?>" class="form form-responsive" enctype="multipart/form-data">
                <div class="card-body">
                  <div class="row form-group">
                    <label for="name" class="col-md-2">Supplier</label>
                    <div class="col-md-4">
                      <select class="form-control select2" name="supplier_ID" id="supplier_ID" style="width:100%" readonly>
                        <?php 
                        if (isset($_GET['purchaseID'])) {
                         $purchase_ID=$_GET['purchaseID'];
                          $getPurchases=getPurchaseByID($purchase_ID);
                          if ( $getPurchases !=0) {
                            foreach ($getPurchases as $getPurchase) { 
                                ...
                                echo $getsuppliers=getSuppliersByID($getPurchase->supplier_ID);
                           if ($getsuppliers !=0) {
                  ...
                  <div class="col-md-4">
                     <input type="date" class="form-control" id="purchase_date" name="purchase_date" placeholder="Date" value="<?php echo $date?>" readonly>
                  </div>
                ...
                <div class="row form-group">
                  ...
                  <div class="col-md-4">
                     <input type="text" class="form-control" id="purchase_details" name="purchase_details" placeholder="Detail" value="<?php echo $purchase_details?>" readonly>
                  </div>
                ...
                <table class="table responsive" >
                 ...
                 $sub_total=0;
                   if (isset($_GET['purchaseID'])) {
                    ...
                    $check= checkifPurchaseReturnTempTablealredyExist($purchaseReturnID,$purchase_ID);
                   $create ='';
                if ($check ==0) {
                   $getDetails= getPurchaseHistoryByPurchaseID($purchase_ID);
                   if ($getDetails !=0) {
                     foreach ($getDetails as $getDetail) {
                       ...
                       if ($getMeds !=0) {
                          foreach ($getMeds as $getMed ) {
                            ...
                            $create=createTempPurchaseReturnDetails($purchase_ID,$purchaseReturnID,$product_name,$product_ID,$return_quantity,$deduction_percentage,$total_deduction,$getDetail->qty,$getDetail->supplier_price,$getDetail->batch_id);
                   ...
                   if ($create =='success') {
                               echo '<script> window.location="add_purchase_return?purchaseID='.$purchase_ID.'&purchaseReturnID='.$purchaseReturnID.'" </script>';
                            }
                 }else{
                   ...
                   if ($getdatas !=0) {
                      foreach ($getdatas as $getdata) {
                        ...
                        $total_deduction +=$getdata->total_deduction ?>
                  <tr>
                    ...
                    <td><?php echo $getdata->total_deduction ?></td>
                    <td ><?php echo $getdata->total_return ?></td>
                    <td><a href="edit_return_purchase_table?id=<?php echo $getdata->id ?>&purchaseID=<?php echo $purchase_ID ?>&returnID=<?php echo $purchaseReturnID ?>" class="btn btn-success btn-sm" ><i class="nav-icon fas fa-edit"></i></a></td>
                  </tr>
                      <?php
                 }
                  }
