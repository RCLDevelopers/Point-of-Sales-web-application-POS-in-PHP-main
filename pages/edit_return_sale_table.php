        ...
        <div class="row">
          ...
          <div class="card card-">
              ...
              if (isset($_POST['submit'])) {
                $id=$_GET['id'];
                $sales_return_ID=$_GET['returnID'];
                 $return_quantity=$_POST['return_quantity'];
                $percentage_deduction=$_POST['percent_deduction'];
                $return_deduction=$_POST['retn_deduction'];
                $total_return=$_POST['retn_total'];

               $update= updateTempSalesReturn($id,$sales_return_ID,$return_quantity,$percentage_deduction,$return_deduction,$total_return);
               if ($update =='success') {
                 echo '<script> window.location="add_sales_return?salesID='.$sales_ID.'&salesReturnID='.$sales_return_ID.'" </script>';
               }

              
            }
              ?>
               <form method="POST" action="edit_return_sale_table?salesID=<?php echo $_GET['salesID'] ?>&id=<?php echo $_GET['id']?>&returnID=<?php echo $_GET['returnID'] ?>" class="form form-responsive" enctype="multipart/form-data">
                <div class="card-body">
                <table class="table" >
                  ...
                  if (isset($_GET['id'])) {
                    # code...
                   $id=$_GET['id'];
                    $getdatas=getTempSalesReturnDetailsByID($id);
                   $sub_total=0;
                   if ($getdatas !=0) {
                  ...
                  <tr>
                    ...
                    <td><input type="text" class="form-control" name="" value="<?php echo $getdata->selling_price ?>" id='selling_price' readonly></td>
                    ...
                    <td ><input type="text" class="form-control" name="" value="<?php echo $getdata->return_deduction ?>" id='return_deduction'  readonly>
                      <input type="hidden" name="retn_deduction" id="retn_deduction">
                    </td>
                    <td ><input type="text" class="form-control" name="" value="<?php echo $getdata->return_total ?>" id='return_total'  readonly>
...
<script type="text/javascript">
  function calculateTotal(){
   ...
   if (return_quantity < quantity ) {
    ...
    var totalReturn= return_quantity * selling_price;
     if (deduction !=0) {
