...
<?php require_once('include/functions/branch.php')?>
<?php require_once('include/functions/preveleges.php')?>
<?php
$role_ID=$user_role;
$privilege_ID='Add User';
        ...
        <div class="row mb-2">
          ...
          <div class="card card-default">
               ...
               <div class="card-header" style="color: #17A589;font-size:">
                ...
                <div class="row">
                  <div class="col-md-4" >
                    <h3 class="card-title" style="color: #17A589;font-size:"><i class="nav-icon fas fa-plus"></i> ADD USER </h3>
                  </div>
                  <div class="col-md-8">
        ...
        <div class="row">
          ...
          <div class="card card-">
              ...
              if (isset($_POST['submit'])) {
               ...
               $checkemail= checkIfUserEmailalreadyExist($email);
               if ($checkemail ==1) {

                 echo '<script> alert("User with that email already exist") </script>';
                ...
                if(move_uploaded_file($tmp_name,$location.$image)){
                      $save=createUser($name,$email,$role,$password,$phone,$image,$branch_ID);

                      if ($save =='success') {
                        $activity='Add new user with name '.$name.' and email '.$email;
                 ...
                 echo '<script> window.location="add_user" </script>';
                    ...
                    echo "not set";
                    //save without image
                   $save=createUserWithoutImage($name,$email,$role,$password,$phone,$branch_ID);
                    if ($save='success') {
                      $activity='Add new user with name '.$name.' and email '.$email;
                 createActivity_logs($activity);
                   echo '<script> alert("User created Successfully") </script>';
              ...
              ?>
              <form method="POST" action="add_user" class="form form-responsive" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <label for="password" class="col-md-2">Password </label>
                   ...
                   <div class="row form-group">
                    ...
                    <?php
                     $getbranchs=getbranchAll();
                     if ($getbranchs !=0) {
                        foreach ($getbranchs as $getbranch ) {?>
                          <option value="<?php echo $getbranch->branch_ID ?>"><?php echo $getbranch->branch_name ?></option>
                        <?php }
                     }
                    ?>
                  </select>
                  </div>
                  <label for="exampleInputEmail1" class="col-md-2">User Role </label>
                  <div class="col-md-4">
