...
$id=$_GET['id'];
 $sales_ID=$_GET['salesID'];
  ...
  $date=$_GET['date'];
 $dec=deleteServiceSalesTempTableDetailsByID($id,$sales_ID);
 if ($dec =='success') {
 	
 	//echo '<script> alert("Role deactivated Successfully") </script>';
                 
   echo '<script> window.location="add_service_sale?salesID='.$sales_ID.'&invoice_no='.$invoice_no.'&date='.$date.'&customer='.$customer_ID.'&det='.$sales_details.'" </script>';
 }else{
